...
ohe = OneHotEncoder()
contextual_info_ohe = ohe.fit_transform(
    df["contextual_info"].values.reshape(-1, 1)
).toarray()
...
for epoch in range(1000):
    inputs = torch.tensor(
        df[["transaction_history", "business_indicators"]].values, dtype=torch.float32
    )
...
with torch.no_grad():
    ...
    df["is_sole_trader"] = predicted_labels

# Save the result
df.to_csv("classified_customer_data.csv", index=False)
